...
/**
 * Joint 1 cross-language e2e: the REAL Trust scorer (FastAPI, apps/02-trust/py)
...
async function spawnTrustScorer(): Promise<{ proc: ReturnType<typeof spawn>; ready: () => Promise<void> }> {
  ...
  const python = spawn('uv', ['run', 'python', '-c', `
...
`], { cwd: '/Users/caiotheodoro/Documents/personal/research/apps/02-trust/py' });
  const ready = () =>
